...
// update dongjin for static function
...
function check_notification(noti_type, noti_id, ref_id){
    $("#notification_item").trigger("click");
    $.ajax({
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        type: "POST",
        url: URL_BASE+"/ajax/check_noti",
        data: {
            'noti_type': noti_type,
            'noti_id': noti_id,
            'ref_id': ref_id
...
function delete_friend_request(e){
    var element     = $(".btn-friend");
    var target_id          = element.attr("data-id"); // target_id
    var txt_add_friend = element.attr('data-unlike');
    var txt_request_friend = element.attr('data-pending');
    var state_friend = element.attr('data-state');
    var data        = 'id=' + target_id;

    if(state_friend == 1){
        element.blur();
        $(this).removeClass("fa fa-times");
        $(this).addClass('icon-spinner2 fa-spin');
        element.removeClass('pending');
        element.find('.text-friend').html(txt_add_friend);
        ...
        $.ajax({
            ...
            type: "POST",
            url: URL_BASE+"/ajax/cancel_friend",
            data: {
                "id" : target_id
            },
            success: function( result ){
                ...
                if( result == '') {
                    ...
                    $(this).removeClass('icon-spinner2 fa-spin');
                    $(element).find("i").addClass('fa fa-user-plus');
                    $(element).find("i").tooltip('hide')
                        .attr('data-original-title', '')
                        .tooltip('show');
                    $(element).find("i").attr('title', '');
                    $(element).find("i").attr('data-original-title', '');
                    element.attr('data-state', result);
                }
            }//<-- RESULT
        });//<--- AJAX
    }
                ...
                if(result.success == "OK"){
                    ...
                    if(result.data.length > 0){
                        for(var i = 0; i < result.data.length; i ++){
                            var src = result.data[i];
                            var dateString = src.created,
                            ...
                            var SRT_MONTH = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sept", "Oct", "Nov", "Dec"];
                            var show_date = SRT_MONTH[date.getMonth()] + ' ' + date.getDate() + ', ' + date.getFullYear();
                            console.log(show_date);
                            html += '<div class="notfication-details"><div class="noty-user-img">';
                            html += '<img src="/public/avatar/'+src.avatar+'" alt="">';
                            html += '</div>';
...
//<---------- * Avatar * ------------>>
    ...
    $(".btn-friend").on('click',function(e){
        var element     = $(this);
        var target_id          = element.attr("data-id"); // target_id
        var txt_my_friend        = element.attr('data-like');
        var txt_add_friend = element.attr('data-unlike');
        var txt_request_friend = element.attr('data-pending');
        var state_friend = element.attr('data-state');
        var data        = 'id=' + target_id;
        ...
        element.find('i').addClass('icon-spinner2 fa-spin');
        switch (state_friend){
            case '2': // cancel friend request
                element.removeClass('active');
                element.find('i').removeClass('fa fa-times').addClass('fa fa-user-plus');
                element.find('.text-friend').html(txt_add_friend);
                break;
            case '1': // not existing
                element.find('i').removeClass('icon-spinner2 fa-spin');
            ...
            case '0': // friend request
                ...
                element.find('i').removeClass('fa fa-user-plus').addClass('fa fa-times');
                element.find('.text-friend').html(txt_request_friend);
                break;
        }
        ...
        $.ajax({
            ...
            success: function( result ){
                ...
                if( result == '') {
                    ...
                    element.attr('data-state', result);
                    if(result == '1'){
                        $(element).find("i").attr('data-original-title', 'Delete Sent Friend Request');
                        // $(element).find("i").tooltip('hide').attr('data-original-title', 'Delete Sent Friend Request').tooltip('show');
                    }else if(result == '0'){
                        $(element).find("i").attr('data-original-title', '');
                        // $(element).find("i").tooltip('hide').attr('data-original-title', 'Delete Sent Friend Request').tooltip('show');
                    }
